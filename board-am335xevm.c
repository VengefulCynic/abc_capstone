...
static struct i2c_board_info lis331dlh_i2c_boardinfo[] = {
	{
		I2C_BOARD_INFO("lis331dlh_accel", 0x18),
	},
};
...
static void lis331dlh_init(int evm_id, int profile)
{
	...
	printk("lis331dlh_init\n");

	/* I2C1 adapter request */
	adapter = i2c_get_adapter(1);
	if (!adapter) {
		printk("ERROR: lis331dlh failed to get adapter i2c1\n");
		return;
	}

	client = i2c_new_device(adapter, lis331dlh_i2c_boardinfo);
	if (!client)
		printk("ERROR: failed to register lis331dlh to i2c1\n");
...
static void setup_starterkit(void)
{
    ...
    printk("ADH: Setting LEDs");
	ret_val = gpio_request(GPIO_TO_PIN(1, 4), "LED1");
	if (ret_val < 0) {
		printk("ADH: Error Setting LEDs");
	} else
	{
		printk("ADH: OK Setting LEDs");
	}

	gpio_direction_output(GPIO_TO_PIN(1, 4), 1);
	gpio_set_value(GPIO_TO_PIN(1, 4), 1);
	mdelay(100);
	gpio_set_value(GPIO_TO_PIN(1, 4), 0);

	gpio_request(GPIO_TO_PIN(1, 5), "LED2");
	gpio_direction_output(GPIO_TO_PIN(1, 5), 1);
	gpio_set_value(GPIO_TO_PIN(1, 5), 1);
	mdelay(100);
	gpio_set_value(GPIO_TO_PIN(1, 5), 0);

	gpio_request(GPIO_TO_PIN(1, 6), "LED3");
	gpio_direction_output(GPIO_TO_PIN(1, 6), 1);
	gpio_set_value(GPIO_TO_PIN(1, 6), 1);
	mdelay(100);
	gpio_set_value(GPIO_TO_PIN(1, 6), 0);

	gpio_request(GPIO_TO_PIN(1, 7), "LED4");
	gpio_direction_output(GPIO_TO_PIN(1, 7), 1);
	gpio_set_value(GPIO_TO_PIN(1, 7), 1);
	mdelay(100);
	gpio_set_value(GPIO_TO_PIN(1, 7), 0);
	printk("ADH: Done Setting LEDs");
}
